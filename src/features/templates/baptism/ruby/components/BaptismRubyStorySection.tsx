...
export function BaptismRubyStorySection({ welcome, reveal }: BaptismRubyStorySectionProps) {
  return (
    ...
    >
      <p className="text-[11px] uppercase tracking-[0.28em] text-[#D4AF37]">Nuestra historia</p>
      <h2 className="mt-4 font-serif text-4xl text-[#2B2B2B] sm:text-5xl">{welcome.title}</h2>
      <div className="mx-auto mt-5 flex w-fit items-center gap-2 text-[#d4af37]">
        <span className="h-px w-8 bg-[#d4af37]/60" />
        <Sparkles className="h-4 w-4" />
        <span className="h-px w-8 bg-[#d4af37]/60" />
      </div>
      <p className="mx-auto mt-5 max-w-2xl text-sm leading-8 text-[#7A7A7A] sm:text-base">{welcome.message}</p>
      <p className="mt-5 font-serif text-2xl text-[#7A7A7A]">{welcome.signature}</p>
    </motion.section>
  );
}
