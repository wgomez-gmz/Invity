    ...
    <motion.section {...reveal(0.16)} className="mt-8 grid gap-4 md:grid-cols-2">
      {dressCode ? (
        <Card className="rounded-[1.8rem] border-[#d4af37]/20 bg-white/92 p-6 shadow-[0_16px_40px_rgba(24,44,76,0.08)]">
          <span className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.24em] text-[#8e6f15]">
            <Shirt className="h-4 w-4" />
            {dressCode.title}
          </span>
          <p className="mt-4 text-sm leading-7 text-slate-600">{dressCode.note}</p>
        </Card>
      ) : null}
      ...
      {gifts ? (
        <Card className="rounded-[1.8rem] border-[#d4af37]/20 bg-white/92 p-6 shadow-[0_16px_40px_rgba(24,44,76,0.08)]">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-600">{gifts.note}</p>
        </Card>
      ) : null}
    </motion.section>
  );
