  ...
  return (
    <motion.section id="details" {...reveal(0.08)} className="mt-8">
      <div className="text-center">
        <p className="text-[11px] uppercase tracking-[0.28em] text-[#D4AF37]">Detalles del evento</p>
        <h2 className="mt-4 font-serif text-4xl text-[#2B2B2B] sm:text-5xl">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {cards.map((card, index) => (
          ...
          >
            <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-[#d4af37]/30 bg-white text-[#D4AF37]">
              <card.icon className="h-4 w-4" />
            </div>
            <p className="mt-4 text-[11px] uppercase tracking-[0.2em] text-[#7A7A7A]">{card.label}</p>
            <p className="mt-2 font-serif text-2xl text-[#2B2B2B]">{card.value}</p>
          </motion.article>
        ))}
      </div>
      ...
      <div className="mt-6 text-center">
        <Button
          ...
          className="h-11 rounded-full border border-[#d4af37]/35 bg-[linear-gradient(135deg,#2B2B2B,#7A7A7A)] px-7 text-[11px] uppercase tracking-[0.2em] text-white shadow-[0_14px_36px_rgba(26,45,71,0.24)] transition hover:-translate-y-0.5 hover:brightness-105"
        >
